...
class SleepManager():
    ...
    def __init__(self, config):
        ...
        if 'sleep_notification_interval' in config:
            self.notification_interval = config['sleep_notification_interval']
    ...
    def _time_checking(self):
        ...
        while True:
            ...
            if self._is_sleep_time(self.sleep_time, self.wake_time, now_s):
                self._update_sleep_state(True)
            else:
                self._update_sleep_state(False)
    ...
    def _update_sleep_state(self, is_sleeping):
        if is_sleeping != self._sleeping:
            if is_sleeping:
